    ...
    try:
        with db_cursor(conn) as cursor:
            ...
            if 'payment_id' in order_data:
                print(f"💳 Step 1: Deleting Payment (ID: {order_data['payment_id']})...")
                cursor.execute("DELETE FROM payments WHERE payment_id = %s", (order_data['payment_id'],))
            ...
            if 'order_items' in order_data:
                ...
                for item in order_data['order_items']:
                    ...
                    if cursor.rowcount > 0:
                        print(f"   ✅ Deleted order item: {item['service_name']}")
                    else:
                        print(f"   ⚠️  Order item not found: {item['service_name']}")
            
            # Step 3: Delete Order
            if 'order_id' in order_data:
                print(f"\n📋 Step 3: Deleting Order (ID: {order_data['order_id']})...")
                cursor.execute("DELETE FROM orders WHERE order_id = %s", (order_data['order_id'],))
                if cursor.rowcount > 0:
                    print("✅ Order deleted")
                else:
            ...
            if 'customer_id' in order_data:
                ...
                cursor.execute("DELETE FROM customers WHERE customer_id = %s", (order_data['customer_id'],))
                if cursor.rowcount > 0:
