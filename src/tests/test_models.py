...
from src.models.customer import get_all_customers, add_customer
from src.models.service import get_all_services
from src.models.category import get_all_categories
from src.models.order_status import get_all_order_statuses
...
def test_models():
    ...
    if services:
        ...
        for service in services[:5]:  # Show first 5
            print(f"   - {service['service_name']}: ₱{service['min_price']}-₱{service['max_price']} {service['price_unit']}")
        if len(services) > 5:
    ...
    if statuses:
        ...
        for status in statuses:
            print(f"   - {status['order_status_name']} (ID: {status['order_status_id']})")
    else:
