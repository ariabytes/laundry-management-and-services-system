    ...
    try:
        with db_cursor(conn) as cursor:
            ...
            order_sql = """
                ...
                VALUES (%s, %s, %s, %s)
            """
            cursor.execute(order_sql, (customer_id, order_status_id, order_date, total_price))
            ...
            cursor.execute("SELECT service_id, service_name, min_price FROM services LIMIT 3")
            ...
            for i, service in enumerate(services):
                ...
                order_item_sql = """
                    ...
                    VALUES (%s, %s, %s, %s)
                """
                cursor.execute(order_item_sql, (order_id, service['service_id'], quantity, price))
                ...
                print(f"   ✅ Added {quantity}x {service['service_name']} - ₱{price:.2f}")
            ...
            cursor.execute("UPDATE orders SET order_status_id = 2 WHERE order_id = %s", (order_id,))
